...
def checkBlackPixel2(pixelArray):
    if pixelArray > BW_THRESHOLD:
        return True
    else:
        return False
...
def convertToBW2(matrix):
    bpmatrix = []
    for z in matrix:
        row = []
        value = 0
        for y in z:
            if (checkBlackPixel2(y)):
                value = 1
            else:
                value = 0
            row += [value]
...
def check_line_candidate(matrix, y, x):
    ...
    if theBoxTotal < boxThreshold and theBoxTotal > boxSize:
        ...
        if twoContinuousOfLess2SegmentsInBox(boxCirumference, boxSize):
            #bt = connectedInBoxTotal(matrix, y, x, boxSize)                     # TOOK THIS OUT AGAIN
...
save_image(bwMatrix, "bwMatrix.jpeg")
...
im= Image.open(FILE_NAME) 
imatrix = numpy.asarray( im, dtype="int32" )
for (y,x) in points_from_line:
    imatrix[y][x] = (255,255,255)
column_index = 0
for row in imatrix:
    row_index = 0
    for pixel in row:
        if pixel[0] > 200 or pixel[1] > 200 or pixel[2] > 200:
            imatrix[column_index][row_index] = (255, 255, 255)
        row_index = row_index + 1
    column_index = column_index + 1
...
img = Image.fromarray( imatrix.astype('uint8'), "RGB" )
img.save( "random.jpeg" )
im= Image.open(FILE_NAME) 
imatrix = numpy.asarray( im, dtype="int32" )
img = Image.fromarray( imatrix.astype('uint8'), "RGB" )
img.save( "random1.jpeg" )
